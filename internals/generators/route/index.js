...
function reducerExists(comp) {
  try {
    fs.accessSync(path.join(__dirname, `../../../client/application/containers/${comp}/reducer.js`), fs.F_OK);
    return true;
  } catch (e) {
    return false;
  }
...
function sagasExists(comp) {
  try {
    fs.accessSync(path.join(__dirname, `../../../client/application/containers/${comp}/sagas.js`), fs.F_OK);
    return true;
  } catch (e) {
    return false;
  }
...
module.exports = {
  ...
  actions: (data) => {
    ...
    if (reducerExists(data.component)) {
      ...
      actions.push({
        type: 'modify',
        path: '../../client/application/routes/routes.js',
        pattern: /(\s{\n\s{0,}path: '\*',)/g,
